...
def process_subject(args):
    ...
    task_id = "imagesTr" if task == "train" else "test"
    ct = data_dir / f"{task_id}" / f"{subject}_0000.nii.gz"
    pt = data_dir / f"{task_id}" / f"{subject}_0001.nii.gz"
...
def create_data_folders(
    ...
    os.makedirs(dest_dir / "imagesTr", exist_ok=True)
    os.makedirs(dest_dir / "labelsTr", exist_ok=True)
    os.makedirs(dest_dir / "labelsTr_Totalsegmentator", exist_ok=True)
    os.makedirs(dest_dir / "imagesTr_resampled", exist_ok=True)
    os.makedirs(dest_dir / "labelsTr_resampled", exist_ok=True)
